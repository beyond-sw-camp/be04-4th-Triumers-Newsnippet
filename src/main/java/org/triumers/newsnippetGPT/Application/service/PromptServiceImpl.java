...
import org.triumers.newsnippetGPT.domain.service.PromptValidationService;
...
public class PromptServiceImpl implements PromptService {
    ...
    @Autowired
    public PromptServiceImpl(PromptValidationService promptValidationService) {
        this.promptValidationService = promptValidationService;
    }

    @Override
    public String createPrompt(NewsDTO news) throws NewsTitleNullException, NewsContentNullException {

        promptValidationService.isNewsNull(news);

        return "너는 시사 평론가이고 나는 너에게 시사 뉴스 기사를 제공해 해당 시사에 대한 문제를 만들고자 한다." +
